...
export function MedicalHistoryPage() {
  ...
  // Local user (replaces useAuth)
  const user = JSON.parse(localStorage.getItem('currentUser') || 'null');

  const storageKey = user
    ? `medical_records_${user.id}`
    : 'medical_records_guest';
  ...
  useEffect(() => {
    ...
    const getFaydaId = () => {
      if (!user) return '';
      if (user.fayda_id) return user.fayda_id;
    ...
    const loadMedicalHistory = async () => {
      try {
        ...
        // simulate backend medical history (fully local)
        const baseRecords =
          demoMedicalHistory[faydaId] || [];
        ...
        const qr = await QRCodeLib.toDataURL(qrData, {
          width: 300
        });
  ...
  const downloadPDF = () => {
    ...
    if (userProfile) {
      ...
      doc.text(
        `Fayda ID: ${userProfile.fayda_id}`,
        20,
        42
      );
      doc.text(
        `Blood Type: ${
          userProfile.blood_type || 'N/A'
        }`,
        20,
        49
      );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* HEADER */}
      <div className="p-6 bg-gradient-to-r from-green-600 to-green-700 text-white rounded-xl">
